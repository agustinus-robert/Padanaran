...
@section('content')
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card mb-4">
                <div class="card-body">
                    <form class="form-block" action="{{ route('administration::curriculas.meets.store', ['semester_id' => $acsem->id, 'next' => request('next', route('administration::curriculas.meets.index', ['academic' => request('academic', $acsem->id)]))]) }}" method="POST"> @csrf
                            ...
                            <div class="col-md-7">
                                <select class="form-control @error('classroom_id') is-invalid @enderror" name="classroom_id" id="classroom_id" required>
                                    ...
                                    @foreach ($acsem->classrooms as $classroom)
                                        <option value="{{ $classroom->id }}" @if (old('classroom_id') == $classroom->id) selected @endif>{{ $classroom->full_name }}</option>
                                    @endforeach
                                </select>
                                @error('category_id')
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-5">
                                {{-- <div class="btn-group-toggle" data-toggle="buttons"> --}}
                                    {{-- @foreach (\Modules\Academic\Models\AcademicSubjectMeet::$colors as $color)
                                        <label class="btn btn-outline-{{ $color }} rounded-circle" style="width: 26pt; height: 26pt;">
                                            <input type="radio" name="props[color]" value="{{ $color }}" autocomplete="off">
                                        </label>
                                    @endforeach --}}
                                    <input type="color" name="props[color]" value="{{ $color ?? '#ffffff' }}" />
                                {{-- </div> --}}
                            </div>
                        </div>
        ...
        <div class="col-md-4">
            <div class="card card-body">
                <form class="form-block" action="{{ route('administration::curriculas.meets.create') }}" method="GET">
