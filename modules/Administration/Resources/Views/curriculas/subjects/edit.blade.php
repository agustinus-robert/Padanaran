				...
				<div class="card-body">
					<form class="form-block" action="{{ route('administration::curriculas.subjects.update', ['subject' => $subject->id, 'next' => request('next', route('administration::curriculas.subjects.index', ['academic' => request('academic', $subject->semester_id)]))]) }}" method="POST"> @csrf @method('PUT')
						...
						<div class="form-group required row mb-3">
							...
							<div class="col-md-7">
								<select class="form-select @error('level_id') is-invalid @enderror" name="level_id" id="level_id" required>
						...
						<div class="form-group required row mb-3">
                            <label for="name" class="col-md-3 col-form-label">Nilai KKM</label>
                            <div class="col-md-4">
                                <input type="number" name="score_standard" class="form-control @error('score_standard') is-invalid @enderror" id="score_standard" placeholder="Nilai KKM" value="{!! $subject->score_standard !!}">
                                @error('score_standard')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                                @enderror
                            </div>
                        </div>
						...
						<div class="form-group row mb-3">
                            ...
                            <div class="col-md-7">
                                <input type="color" name="color_id"  value="{!! $subject->color_id !!}" />
                                @error('color_id')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                                @enderror
                            </div>
