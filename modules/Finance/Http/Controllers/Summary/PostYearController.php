...
class PostYearController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if (is_null($defaultTemplate) || $defaultTemplate->isEmpty()) {
            return redirect()->back()->with(
                'danger',
                'Belum ada template gaji ke 13, silakan buat template terlebih dahulu!'
            );
        }
    ...
    {
        $employeed = Employee::find($request->employee);
        
        $summary = array_merge(
            $request->transformed()->toArray(),
            [
                'type' => DataRecapitulationTypeEnum::G13,
                'empl_id' => $employeed->id
            ]
        );

        if (EmployeeDataRecapitulation::updateOrCreate(Arr::only($summary, ['start_at', 'end_at', 'empl_id', 'type']), $summary)) {
            $request->user()->log('melakukan rekapitulasi Gaji ke-13 <strong>' . $employeed->user->name . '</strong> untuk cut off tanggal <strong>' . $request->input('start_at') . ' s.d. ' . $request->input('end_at') . '</strong>', Employee::class, $employeed->id);
            return redirect()->next()->with('success', 'Rekapitulasi Gaji ke-13 <strong>' . $employeed->user->name . '</strong> untuk cut off tanggal <strong>' . $request->input('start_at') . ' s.d. ' . $request->input('end_at') . '</strong> berhasil disimpan');
        }
