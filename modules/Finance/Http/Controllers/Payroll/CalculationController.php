...
class CalculationController extends Controller
{
    ...
    function convertJsonPathToPostgres(string $path): string
    {
        $parts = explode('->', $path);
        $base = array_shift($parts);
        foreach ($parts as $i => $part) {
            $quoted = "'$part'";
            $base .= ($i === count($parts) - 1) ? "->>$quoted" : "->$quoted";
        }
        return "($base)::numeric";
    }
                    ...
                    case 'MODEL':
                        ...
                        foreach (($component->meta->algorithm?->models ?? []) as $model => $x) {
                            ...
                            if (isset($x->after)) {
                                ...
                                if (isset($x->action)) {
                                    if (str_contains($x->action_column, '->')) {
                                        $sql = $this->convertJsonPathToPostgres($x->action_column);

                                        $amount += $query
                                            ->selectRaw("COALESCE(SUM($sql), 0)")
                                            ->value('coalesce');
