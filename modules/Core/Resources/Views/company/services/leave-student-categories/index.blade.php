            ...
            <section>
                <div class="card border-0">
                    ...
                    <div class="card-body border-top border-light">
                        <form class="form-block row g-2" action="{{ route('core::company.services.leave-student-categories.index') }}" method="get">
                            ...
                            <div class="col-auto">
                                <a class="btn btn-light" href="{{ route('core::company.services.leave-student-categories.index', request()->only('trashed', 'closed')) }}"><i class="mdi mdi-refresh"></i> <span class="d-sm-none">Reset</span></a>
                            </div>
                            <div class="col-auto">
                                        ...
                                        <td class="py-2 text-end" nowrap>
                                            @if ($category->trashed())
                                                @can('restore', $category)
                                                    <form class="form-block form-confirm d-inline" action="{{ route('core::company.services.leave-student-categories.restore', ['category' => $category->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('put')
                                                        <button class="btn btn-soft-info rounded px-2 py-1" data-bs-toggle="tooltip" title="Pulihkan"><i class="mdi mdi-refresh"></i></button>
                                                    </form>
                                                @endcan
                                            @else
                                                @can('update', $category)
                                                    <a class="btn btn-soft-warning rounded px-2 py-1" href="{{ route('core::company.services.leave-student-categories.show', ['category' => $category->id, 'next' => url()->current()]) }}" method="post" data-bs-toggle="tooltip" title="Ubah"><i class="mdi mdi-pencil-outline"></i></a>
                                                @endcan
                                                @can('destroy', $category)
                                                    <form class="form-block form-confirm d-inline" action="{{ route('core::company.services.leave-student-categories.destroy', ['category' => $category->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('delete')
                                            ...
                                            @if (!request('trash'))
                                                @can('store', Modules\Core\Models\CompanyDepartment::class)
                                                    <div class="mb-lg-5 mb-4 text-center">
                                                        <a class="btn btn-soft-danger" href="{{ route('core::company.services.leave-student-categories.create', ['next' => url()->current()]) }}"><i class="mdi mdi-plus"></i> Buat kategori baru</a>
                                                    </div>
                                                @endcan
                                            @endif
                                        </td>
