...
class ManageSubmissionScheduleController extends Controller
{
    ...
    public function importExcel(Request $request)
    {
        ...
        //     return redirect()->back()->with('danger', 'File excel schedule masih kosong.');
        // }
        ...
        for ($col = 1; $col <= $lastColumnIndex; $col += 5) {
            ...
            if ($tanggalValue) {
                ...
                if (is_numeric($tanggalValue)) {
                    // Format date dari Excel (misal: 44927)
                    $tanggal = Carbon::instance(Date::excelToDateTimeObject($tanggalValue))->format('Y-m-d');
                } else {
                    // Format general (misal: 31/12/2025 atau 2025-12-31)
                    $tanggalValue = trim($tanggalValue);

                    if (str_contains($tanggalValue, '/')) {
                        $tanggal = Carbon::createFromFormat('d/m/Y', $tanggalValue)->format('Y-m-d');
                    } else {
                        $tanggal = Carbon::parse($tanggalValue)->format('Y-m-d');
                    }
                }
                ...
                //     // if ($tanggal < $lastMonday) {
                //     //     return redirect()->back()->with('danger', 'Data gagal diproses, pastikan tanggal sesuai dengan realisasi 1 minggu sebelumnya!');
                //     // } else {
        ...
        foreach ($dateShifts as $key => $val) {
            ...
            foreach ($val as $key2 => $val2) {
                foreach ($val2 as $values) {
                    preg_match('/^([a-zA-Z0-9]+)([+-]\d+)?$/', $values, $matches);

                    $metaCode = DB::table('empl_meta')
                        ->where('key', 'code')
                        ->where('value', $matches[1])
                        ->first();
