...
use Modules\HRMS\Http\Requests\Service\Teacher\UpdateRequest;
...
class TeacherController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $division = null;
        if ($emp->contract->positions->first()->position->dept_id == 6 || $emp->contract->positions->first()->position->dept_id == 7) {
            $division = $emp->contract->positions->first()->position->dept_id;
        }

        $teachers = Employee::isTeacher($division)->with('user.meta', 'contract.positions.position')
