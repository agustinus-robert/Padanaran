...
@section('title', 'Ubah pengaturan template gaji | ')
@section('navtitle', 'Ubah pengaturan template gaji')
        ...
        <div class="col-9">
            <div class="d-flex align-items-center mb-4">
                ...
                <div class="ms-4">
                    ...
                    <div class="text-secondary">Isi formulir dibawah untuk mengubah pengaturan</div>
                </div>
            </div>
            <div class="card border-0">
                ...
                <div class="card-body border-top border-light p-4">
                    <form class="form-block form-confirm" action="{{ route('hrms::payroll.configs.update', ['config' => $config->id, 'next' => request('next', route('hrms::payroll.configs.index'))]) }}" method="POST"> @csrf @method('put')
                        <div class="mb-3">
                            <label for="key" class="form-label">Name</label>
                            <input type="text" class="form-control" name="key" id="key" aria-describedby="helpId" placeholder="" value="{{ $config->key }}" />
                                ...
                                @foreach ($components->groupBy('slip.name') as $key => $_components)
                                    <optgroup label="{{ $key ?: 'Lainnya' }}">
                                        @forelse($_components as $key => $component)
                                            <option value="{{ $component->id }}" @selected($component->id == $config->meta?->component)>{{ $component->name }}</option>
                                        @empty
                                        @endforelse
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <label for="calculation" class="form-label">Kalkulasi</label>
                            <input type="text" class="form-control" name="calculation" id="calculation" aria-describedby="calc" value="{{ $config->meta?->calculation }}" />
