...
@section('title', isset($contract) ? 'Ubah perjanjian kerja | ' : 'Tambah perjanjian kerja | ')
@section('navtitle', isset($contract) ? 'Ubah perjanjian kerja' : 'Tambah perjanjian kerja')
        ...
        <div class="col-xxl-8 col-xl-10">
            ...
            <div class="card mb-4 border-0">
                <x-card-header type="{{ config('theme.default') }}">
                    {{ isset($contract) ? 'Edit Perjanjian Kerja' : 'Tambah Perjanjian Kerja Baru' }}
                </x-card-header>

                <div class="card-body shadow-sm">
                    <form
                        class="form-block"
                        action="{{ isset($contract) ? route('hrms::employment.contracts.update', ['contract' => $contract->id, 'next' => request('next')]) : route('hrms::employment.contracts.store', ['next' => request('next')]) }}"
                        method="POST"
                    ...
                    >
                        ...
                        <x-input-group required>
                            ...
                            <x-select
                                name="employee_id"
                                :value="old('employee_id', $contract->employee_id ?? null)"
                        ...
                        <x-input-group required>
                            ...
                            <x-select
                                name="contract_id"
                                :value="old('contract_id', $contract->contract_id ?? null)"
                        ...
                        <x-input-group required>
                            ...
                            <x-input name="kd" :value="old('kd', $contract->kd ?? null)" required />
                        ...
                        <x-input-group :isRow="true" required>
                            ...
                            <x-col size="6">
                                <x-input type="datetime-local" name="start_at" :value="old('start_at', isset($contract) && $contract->start_at ? $contract->start_at->format('Y-m-d\TH:i') : null)" required />
                            </x-col>
                            <x-col size="6">
                                <x-input type="datetime-local" name="end_at" :value="old('end_at', isset($contract) && $contract->end_at ? $contract->end_at->format('Y-m-d\TH:i') : null)" />
                            </x-col>
                        ...
                        <x-input-group  :isRow="true" required>
                            ...
                            <x-radio-group
                                ...
                                :selected="old('work_location', $contract->work_location ?? null)"
                            />
