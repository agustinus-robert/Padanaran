    ...
    <div class="row">
        <div class="col-xl-8">
            <div class="card border-0">
                <div class="card-body">
                    <i class="mdi mdi-format-list-bulleted"></i> Rekapitulasi mengajar guru
                </div>
                        ...
                        <tbody>
                            @forelse($employees as $employee)
                                ...
                                <tr @class(['table-active text-muted' => is_null($employee->contract)])>
                                    ...
                                    <td>
                                        @if ($summary->count())
                                            <a class="btn btn-soft-warning rounded px-2 py-1" href="{{ route('hrms::summary.teachings.show', ['teaching' => $employee->id, 'start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d'), 'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Rekap baru"><i class="mdi mdi-pencil-outline"></i></a>
                                        @else
                                            <a class="btn btn-soft-primary rounded px-2 py-1" href="{{ route('hrms::summary.teachings.create', ['employee' => $employee->id, 'start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d'), 'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Rekap baru"><i class="mdi mdi-plus-circle-outline"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                ...
                <div class="card-body border-top">
                    <form class="form-block" action="{{ route('hrms::summary.teachings.index') }}" method="get">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" name="search" placeholder="Cari nama atau Kode guru ..." value="{{ request('search') }}" />
                        </div>
