                                ...
                                @foreach ($employees as $employee)
                                    <tr>
                                        ...
                                        <td nowrap>
                                            ...
                                            <div class="small text-muted"> {{ optional(optional(optional($employee->contract)->position)->position)->name ?? '-' }}</div>
                                        </td>
                                                ...
                                                $days = $employee->schedules
                                                    ->map(function ($schedule) use ($start_at, $end_at) {
                                                        return collect($schedule->dates)
                                                            ->filter(function ($times, $date) use ($start_at, $end_at) {
                                                                return $start_at->lte($date) && $end_at->gte($date);
                                                            })
                                                            ->map(function ($date) {
                                                                return count(
                                                                    array_filter($date, function ($times) {
                                                                        return count(array_filter($times)) == 2;
                                                                    }),
                                                                );
                                                            })
                                                            ->sum();
