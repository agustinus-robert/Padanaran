                                       ...
                                       <td>
                                        ...
                                        <td class="py-2 text-end" nowrap>
                                            @if ($employee->contract)
                                                @if ($schedule)
                                                    @can('show', $schedule)
                                                        <a class="btn btn-soft-primary rounded px-2 py-1" href="{{ route('hrms::service.teacher.schedule.show', ['schedule' => $employee->id, 
                                                        'start_at' => request('start_at'),
                                                        'end_at'   => request('end_at'),
                                                        'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Kelola"><i class="mdi mdi-eye-outline"></i></a>
                                                    @endcan
                                                @else
                                                    @can('store', Modules\HRMS\Models\EmployeeSchedule::class)
                                                        <a class="btn btn-soft-primary rounded px-2 py-1" href="{{ route('hrms::service.teacher.schedule.create', ['employee' => $employee->id, 'month' => request('month', date('Y-m')), 'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Buat baru"><i class="mdi mdi-plus-circle-outline"></i></a>
                                                    @endcan
                                                @endif
                                                @can('destroy', $schedule)
                                                    @if($schedule)
                                                        <form class="form-block form-confirm d-inline" action="{{ route('hrms::service.teacher.schedule.destroy', ['schedule' => $schedule->id, 'next' => url()->full()]) }}" method="post"> @csrf @method('delete')
