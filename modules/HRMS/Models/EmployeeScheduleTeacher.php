...
class EmployeeScheduleTeacher extends Model
{
    ...
    public function lesson($codeLesson)
    {
        return AcademicSubject::find($codeLesson)->first();
    }
    ...
    public function getEntryLogs(Collection $scanlogs, $withNull = false)
    {
        ...
        foreach ($this->getAttribute('dates') as $date => $data) {
            if (!isset($data['shifts']) || !is_array($data['shifts'])) {
                continue; // skip kalau tidak ada shifts
            }

            $previousOutTime = null;

            foreach ($data['shifts'] as $i => $shift) {
                $workshift = EmployeeScheduleLesson::where('category_schedule_id', $data['category_id'] ?? 1)
                    ->orWhere(function ($q) {
                        $q->where('category_schedule_id', 2);
                    })
                    ->get(['id', 'in', 'out'])
                    ->map(function ($item) {
                        return [
                            'in'  => Carbon::parse($item->in)->format('H:i'),
                            'out' => Carbon::parse($item->out)->format('H:i'),
                        ];
                    })
                    ->toArray()[$i] ?? null;
                ...
                if ($withNull ? $shift : array_filter($shift)) {
                    $inTime = Carbon::parse($workshift['in']);
                    $outTime = Carbon::parse($workshift['out']);
                    ...
                    $out = isset($scanlogs[$date]) && count($scanlogs[$date]) > 1
                        ? $scanlogs[$date]->last()->created_at
                        : null;
                    ...
                    $entries[$date][] = (object) [
                        ...
                        'shift' => $workshift,
                        'schedule' => array_map(
                            fn($v) => is_string($v) ? Carbon::parse($v) : null,
                            array_filter($shift, fn($k) => is_int($k), ARRAY_FILTER_USE_KEY)
                        ),
                        'lesson' => $shift['lesson'][0] ?? null,
