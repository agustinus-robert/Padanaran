...
@section('main')
    <div class="relative">
        <div class="-z-20 bg-cover bg-center h-[50vh] absolute top-0 w-full"
            style="background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('{{ get_image($program_detail->location, $program_detail->image) }}');">
        </div>
        <div class="container mx-auto space-y-8 mb-16">
            <header class="flex items-center pt-[10vh]">
                <div class="space-y-2 max-w-4xl">
                    ...
                    <h1 class="font-bold text-white text-5xl">{{get_content_json($program_detail)[$lang]['title']}}
                    </h1>
            ...
            <div
                ...
                <div class="flex items-center gap-2">
                    ...
                    {!! tgl_indo(date('Y-m-d', strtotime($program_detail->created_at))).' '.date('H:i', strtotime($program_detail->created_at)) !!}
                </div>
                {!! get_content_json($program_detail)[$lang]['post2'] !!}
            </div>
        </div>
                ...
                <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($link_related as $key => $article)
                        <article class="max-w-xs">
                            <a href="{{ url($lang . '/works/' . get_content_json($article)[$lang]['slug']) }}">
