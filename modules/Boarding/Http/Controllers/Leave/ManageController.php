...
namespace Modules\Boarding\Http\Controllers\Leave;
...
use Modules\Boarding\Notifications\Leave\Submission\SubmissionNotification;
use Modules\Boarding\Notifications\Leave\Manage\ApprovedNotification;
...
class ManageController extends Controller
{
	...
	//	$employee = $user->employe;
		...
		return view('boarding::leave.manage.index', compact('user', 'leaves', 'pending_leaves_count'));
	}
	...
	public function show(BoardingStudentsLeave $leave, Request $request)
	{
		...
		return view('boarding::leave.manage.show', compact('user', 'student', 'employee', 'leave', 'results', 'parentBoard'));
	}
	...
	public function update(BoardingCompanyApprovable $approvable, UpdateRequest $request)
	{
		...
		if ($request->input('result') == ApprovableResultEnum::APPROVE->value) {
			...
			if ($superior = $approvable->modelable->approvables->sortBy('level')->filter(fn ($a) => $a->level > $approvable->level)->first()) {
				...
				Auth::user()->log(
					'Perizinan disetujui oleh <strong>' . ($superior->user->name ?? '-') . '</strong>' .
		...
		if ($request->input('result') == ApprovableResultEnum::REJECT->value) {
			...
			if ($superior = $approvable->modelable->approvables->sortBy('level')->filter(fn ($a) => $a->level > $approvable->level)->first()) {
				Auth::user()->log(
					'Perizinan ditolak oleh <strong>' . ($superior->user->name ?? '-') . '</strong>' .
					' <strong>[ID: ' . $superior->id . ']</strong>',
					BoardingStudentsLeave::class,
					$superior->id
				);
