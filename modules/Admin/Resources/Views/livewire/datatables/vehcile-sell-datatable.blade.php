    ...
    <div class="mt-4 container">
     ...
     @push('scripts')
        <script type="text/javascript">$(function(){window.LaravelDataTables=window.LaravelDataTables||{};
            ...
            window.LaravelDataTables["vehcile-sell-table"]=$("#vehcile-sell-table").DataTable({"serverSide":true,"processing":true,"ajax":{"url":"datatable?class=Modules\\Admin\\DataTables\\vehcileSellDatatables","type":"GET","data":function(data) {
                ...
                return $.extend(data, myData)
                          ...
                          var page_info = window.LaravelDataTables["vehcile-sell-table"].page.info();
                          ...
                          $('#pagelist').val(page_info.page);
                                ...
                                var trash = "{{$trash}}"
                                    if(trash == 1){
                                        return '<button class="btn btn-sm btn-info" wire:click="$dispatch(\'vehcile-sell-restore\', { id: '+row.id+'})"><i class="fa fa-refresh" aria-hidden="true"></i></button>'
                                    } else {
                                        return '<span class="badge bg-warning fw-normal">Penjualan Berhasil</span>';
                                    //     return '<button class="btn btn-sm btn-warning" wire:click="$dispatch(\'vehcile-sell-edited\', { id: '+row.id+'})"><i class="fa fa-pencil" aria-hidden="true"></i></button>'+
                                    // '<button class="btn btn-sm btn-danger" wire:click="$dispatch(\'vehcile-sell-delete\', { id: '+row.id+'})"><i class="fa fa-trash" aria-hidden="true"></i></button>';    
                                    }
                            }
            ...
            $('#pagelist').change(function(){
                ...
                var test_table = $('#vehcile-sell-table').dataTable();
