                                    ...
                                    <tr @if ($user->trashed()) class="table-light text-muted" @endif>
                                        ...
                                        <td class="py-2 text-end" nowrap>
                                            @if ($user->isnot(Auth::user()))
                                                ...
                                                @else
                                                    @can('show', $user)
                                                        <a class="btn btn-soft-primary rounded px-2 py-1" href="{{ route('admin::system.users.show', ['user' => $user->id, 'page' => 'profile', 'next' => url()->full()]) }}" method="post" data-bs-toggle="tooltip" title="Lihat detail"><i class="mdi mdi-eye-outline"></i></a>
                                                    @endcan
                                                    @can('destroy', $user)
                                                        <form class="form-block form-confirm d-inline" action="{{ route('admin::system.users.destroy', ['user' => $user->id, 'next' => url()->full()]) }}" method="post"> @csrf @method('delete')
