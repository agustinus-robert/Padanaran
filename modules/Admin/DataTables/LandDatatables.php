...
class LandDatatables extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('address', function($row){
                return $row->address_primary;
            })
            ->addColumn('certificate', function($row){
                return '<b>Pinjam Pakai</b>'.
                '<br />'.
                ($row->right_of_user == '1' ? 'Hak Pakai' : 'Hak Pengelolaan').
                '<br />'.
                '<b>'.'<b>Dokumen</b>'.
                '<br />'.
                '<a href="'.asset($row->certificate_file).'">Download</a>';
            })
            ->rawColumns(['link', 'address', 'certificate']);
