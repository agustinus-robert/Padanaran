...
use Modules\Admin\Models\PostFBuilderD;
...
class PostingFormListDatatables extends DataTable
{
    /**
        ...
        return datatables()
            ...
            ->addColumn('action', function($row) use ($trash){
                 ...
                 if($trash == 1){
                     ...
                     $template = '';
                     // $template .= view('admin::layouts_master.component.button_list_form', array('id' => $row->id, 'lists' => route('admin::builder.list_posting_forms', ['posting_forms' => '?id_menu='.$row->menu_id.'&post_id='.$row->id])))->render();
     ...
     */
    public function query(PostFBuilderD $model, Request $request)
    {
        ...
        if(!empty($qs)){
            ...
            $data = $model::where('post_form_id', $output['post_id'])->where('deleted_at', null);
