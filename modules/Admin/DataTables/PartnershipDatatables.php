    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function($row){
                $template = '';
                $template .= view('admin::layouts_master.component.button_status_approval', array('approve_id' => $row->id, 'status' => $row->status))->render();

                if($row->status !== 2){
                    $template .= view('admin::layouts_master.component.button_status_reject', array('approve_id' => $row->id))->render();
                }
